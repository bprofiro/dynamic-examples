...
Supplying requires two transactions: approve the mToken to spend USDC, then `mint`. Withdrawing needs no approval. Use viem directly with `createWalletClientForWalletAccount` — the wallet client is configured from the wallet account's active network, so assert it is on Base before writing:
