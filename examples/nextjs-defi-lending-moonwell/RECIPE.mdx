...
<Info>
  ...
  **Chain the supply onto the approval only when the allowance is actually readable.** Wait for it after the receipt; if it appears, continue straight into the supply, and if it does not, stop and let the user press Supply — a second click costs them a tap, whereas a supply sent against an invisible allowance costs them a failed transaction and its gas.
